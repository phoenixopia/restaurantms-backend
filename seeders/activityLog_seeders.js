// seeders/activity-log-seeder.js
"use strict";

const { ActivityLog, User, Customer } = require("../models/index");

module.exports = async () => {
  try {
    console.log("Seeding ActivityLogs...");

    // Fetch some users and customers to reference
    const users = await User.findAll({ limit: 3 });
    const customers = await Customer.findAll({ limit: 3 });

    if (users.length === 0 || customers.length === 0) {
      console.log("⚠️ No users or customers found. Skipping ActivityLog seeding.");
      return;
    }

    // Create sample activity logs
    const logs = [
      {
        user_id: users[0].id,
        customer_id: customers[0].id,
        module: "Reservations",
        action: "Created a new reservation",
        details: { table: 5, time: "19:00" },
      },
      {
        user_id: users[1].id,
        customer_id: customers[1].id,
        module: "Customers",
        action: "Updated customer info",
        details: { field: "email", old: "old@example.com", new: "new@example.com" },
      },
      {
        user_id: users[2].id,
        customer_id: customers[2].id,
        module: "Menus",
        action: "Added a new menu item",
        details: { item: "Margherita Pizza", price: 12.5 },
      },
    ];

    await ActivityLog.bulkCreate(logs);

    console.log("✅ ActivityLogs seeded successfully");
  } catch (error) {
    console.error("❌ Failed seeding ActivityLogs:", error);
  }
};
