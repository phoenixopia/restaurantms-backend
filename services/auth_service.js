...
const AuthService = {
  ...
  async login({
    ...
    try {
      ...
      if (signupMethod === "phone" && !user.phone_verified_at)
        throwError("Phone not confirmed.");
      ...
      if (user.twoFA?.is_enabled) {
        if (!code) throwError("2FA code required.");
  ...
  async verifyCode({ emailOrPhone, signupMethod, code }, url, res) {
    ...
    try {
      ...
      if (!user) throwError("Invalid or expired code.");
  ...
  async resendCode({ emailOrPhone, signupMethod }) {
    ...
    try {
      ...
      if (user.email_verified_at || user.phone_verified_at)
        throwError(`${signupMethod} already verified`);
  ...
  async forgotPassword({ emailOrPhone, signupMethod }) {
    ...
    try {
      ...
      if (
        ...
        throwError(`${signupMethod} not verified`);
  ...
  async resetPassword({ code, newPassword, signupMethod, emailOrPhone }) {
    ...
    try {
      ...
      if (!user) throwError("Invalid or expired reset code.");
